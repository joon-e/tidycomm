...
- `recode_scale()`: Recodes numeric scales into categorical variables based on provided breaks and labels.
...
WoJ %>% 
  ...
  recode_cat_scale(country, assign = c("Germany" = "german", "Switzerland" = "swiss", .else = "other"))
```
...
WoJ %>%
  dplyr::select(autonomy_emphasis) %>%
  recode_scale(autonomy_emphasis, breaks = c(0, 1, 4, Inf), labels = c("Low", "Medium", "High"))
```
